...
def download_videos_with_delay(start_index, end_index):
    # Download videos from the playlist within the specified range
    subprocess.run(["yt-dlp", "--playlist-items", f"{start_index}-{end_index}", playlist_url])
...
for i in range(1, total_videos + 1, batch_size):
    # Calculate the end index for the current batch
    end_index = min(i + batch_size - 1, total_videos)
